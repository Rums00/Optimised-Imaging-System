...
class FrescoCamera:
    ...
    def set_gain(self, gain_value: float):
       self.camera.GainAuto.SetValue(PySpin.GainAuto_Off)  # Disable auto gain
       gain_value_to_set = self.__clip(gain_value,
                                       self.camera.Gain.GetMin(),
                                       self.camera.Gain.GetMax())
       self.camera.Gain.SetValue(gain_value_to_set)
