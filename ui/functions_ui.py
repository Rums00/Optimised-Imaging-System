...
class Functions(Frame):
    ...
    def init_ui(self):
        ...
        gain_button = tk.Button(self,
                                          text='Gain',
                                          command=self.open_gain)
        gain_button.grid(column=2, row=1, ipadx=2, pady=2, sticky=tk.W)
        ...
        protocols_performer_button.grid(column=1, row=2, ipadx=2, pady=2, sticky=tk.W)

        exposure_button = tk.Button(self,
                                    text='Exposure',
                                    command=self.open_exposure)
        exposure_button.grid(column= 1, row=1, ipadx=2, pady=2, sticky=tk.W)
 ...
 ##new code
    def open_gain(self):
        new_window = Toplevel(self)
        new_window.title("Gain")
        new_window.geometry("400x250")
        GainUI(new_window, z_camera=self.z_camera).pack()
